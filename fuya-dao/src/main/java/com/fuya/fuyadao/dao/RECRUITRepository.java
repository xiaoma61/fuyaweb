...
public interface RECRUITRepository extends JpaRepository<RECRUIT,Integer>, JpaSpecificationExecutor<RECRUIT> {
    ...
    @Transactional//注解，使用默认配置，抛出异常之后，事务会自动回滚，数据不会插入到数据库
    @Modifying
    @Query("update RECRUIT r set r.NUMS=?1 ,r.RDESCRIBE=?2,r.EDUCATION=?3,r.ENDTIME=?4,r.WORKBACKGROUND=?5,r.HIGHLIGHT=?6,r.LINKMAN=?7,r.PHONE=?8,r.POSITION=?9,r.SALARY=?10,r.STARTTIME=?11," +
    ...
    @Query(value = "SELECT * FROM RECRUIT  WHERE USERSID = ?1 ORDER BY ?#{#pageable}",
            nativeQuery = true)
