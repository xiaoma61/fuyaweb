(function(){
	...
	function success2(){
		$.ajax({
			...
			success:function(data){
				...
				$('.n_picture').eq(0).html('<img src="../../img/y1.jpg" height="200px" alt="" id="imgID"/>');
	...
	function success3(){
		$.ajax({
			...
			success:function(data){
				...
				for(var i=0;i<2;i++){
					var d;
					d='<div class="certify"><div class="certify_picture"><img src="../../img/certificate.jpg" width="180px"  alt=""/></div><div class="certify_title">健康证明</div></div>';
					$('.certify_content').append(d);
				}
	...
	function success4(){
		...
		$('.n_picture').eq(0).html('<img  height="200px" alt="" id="img"/>');
		$('#imgfun').html('<div class="file-box"><label for="fil">上传图片</label><input type="file" name="fil" class="file-btn" id="fi" onchange="imgfun(fi,img)"></div>');
		$('.certify_content').html("");
		$('.certify_content').append('<div class="certify" id="add1"><div class="certify_add"><input type="file" name="pict" class="add-btn" id="pict"  onchange="addimg()"></div></div><div style="clear:both"></div>');
	}
	...
	$(document).on('click','#add a:last',function(){
		$("#excel").click();
	});
